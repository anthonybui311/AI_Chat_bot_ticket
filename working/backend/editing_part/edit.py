...
def _process_update_data(update_data: Dict[str, Any], stage_manager) -> Tuple[str, str]:
    ...
    Args:
        Cập nhật ticket thành công!: Dictionary containing update information
        stage_manager: Stage management object
    Returns:
        Tuple of (response, summary)
    """
...
Cảm ơn bạn đã liên hệ. 👋 Tạm biệt! Hẹn gặp lại bạn trong các cuộc trò chuyện sau."""
...
def _handle_ticket_not_found(ticket_id: str) -> Tuple[str, str]:
    ...
    response = (f"""❌ Không tìm thấy ticket '{ticket_id}' trên hệ thống. 
                Anh chị vui lòng kiểm tra lại thông tin và liên hệ lại sau. Em cảm ơn, em chào anh chị.!""")
    return response, "thoát"
...
def _handle_update_api_error(ticket_id: str, result: Dict[str, Any]) -> Tuple[str, str]:
    ...
    response = f"""❌ Không thể cập nhật ticket {ticket_id}. 
                Lỗi hệ thống: {error_code} - {error_message}. 
                Rất xin lỗi anh chị, do hệ  thống đang gặp sự cố, em chưa thể sửa thông tin ticket cho anh chị được. 
                Anh chị vui lòng thực hiện lại sau. Cảm ơn anh chị, chào tạm biệt anh chị!"""
